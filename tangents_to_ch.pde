...
void draw() {
    ...
    poly = convex_hull(points_set.getPoints());
    poly.col = green;
    points_set.draw();
    poly.draw();
    if (q != null) {
...
void mouseClicked() {
    if (mouseButton == LEFT)
        points_set.add(new Point(mouseX, mouseY));
    ...
    void ccw_angle_sort(int i) {
        ...
        while (this.points.size() > 0) {
            ...
            for (int j = 1; j < this.points.size(); ++j) {
                ...
                float deltaX_j = (this.points.get(j).x - pivot.x);
                float deltaY_j = (this.points.get(j).y - pivot.y);

                if (atan2(deltaY_j, deltaX_j) < atan2(deltaY_p, deltaX_p)) {
                    p = this.points.get(j);
                } else if (atan2(deltaY_j, deltaX_j) == atan2(deltaY_p, deltaX_p) && this.points.get(j).x > p.x) {
                    p = this.points.get(j);
                }
            }
            sorted.add(p);
...
Boolean is_cw(Point a, Point b, Point c) {
...
Boolean is_in_triangle(Point a, Point b, Point c, Point p) {
    Boolean sens = is_cw(a, b, c);
    Point[] tri = {a, b, c};
    for (int i = 0; i < 3; i++) {
...
Polygon convex_hull_chan(ArrayList<Point> points, int m, int h) {
    ...
    SetOfPoints p = new SetOfPoints(points);
    ArrayList<Polygon> sub_p_set = new ArrayList();

    int c = ceil(float(n)/m);
    for (int j = 0; j < c; ++j) {
        sub_p_set.add(convex_hull_gs(p.subset_p(j, m)));
    }
    ...
    for (int k = 0; k < h; ++k) {
        ...
        for (int i = 0; i < c; ++i) {
            Boolean r = sub_p_set.get(i).remove(ch.last());
            if (sub_p_set.get(i).size() > 0)
                q_set.add(lower_tangent(sub_p_set.get(i).getPoints(), ch.last()));
            if (r == true)
                sub_p_set.get(i).add(ch.last());
        }
        Point new_p = q_set.remove(0);
        while (q_set.size() > 0) {
            Point q = q_set.remove(0);
            if(is_ccw(ch.get(ch.size()-2), ch.last(), q) && is_cw(ch.last(), new_p, q))
                new_p = q;
        }
...
Polygon convex_hull(ArrayList<Point> points) {
    ...
    for (int m = 2, i = 0; ; ++i, m = round(pow(2, pow(2, i)))) {
...
Boolean is_in_ch(ArrayList<Point> points, Point q) {
...
Point upper_tangent(ArrayList<Point> points, Point q) {
    ...
    Polygon ch = convex_hull_gs(points);
    ArrayList<Point> p = ch.getPoints();
    int n = p.size();
    int a = 0;
    int b = n;
